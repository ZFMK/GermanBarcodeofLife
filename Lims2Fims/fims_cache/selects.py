...
class queries():
	def __init__(self, project=1):
		if project==1:
			...
			self.project = "p.ProjectID=29992"
	...
	def dwb_all(self):
		...
		FROM CollectionSpecimen AS c
			LEFT JOIN FirstLinesCollectionAgent AS ca ON ca.CollectionSpecimenID=c.CollectionSpecimenID
			LEFT JOIN IdentificationUnit AS iu ON (iu.CollectionSpecimenID=c.CollectionSpecimenID AND iu.DisplayOrder=1)
			LEFT JOIN IdentificationSequenceMax im on im.CollectionSpecimenID=c.CollectionSpecimenID
			LEFT JOIN Identification i on (im.IdentificationUnitID=iu.IdentificationUnitID and i.CollectionSpecimenID=iu.CollectionSpecimenID and im.IdentificationSequenceMax=i.IdentificationSequence)
