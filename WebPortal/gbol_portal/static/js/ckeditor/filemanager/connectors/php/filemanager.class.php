...
class Filemanager {
  ...
  private function get_file_info($path='',$return=array()) {
    ...
    if(is_dir($this->doc_root . $path)) {
      ...
      $this->item['preview'] = '/download?newsUpload=1&filename=' . rawurlencode($this->item['filename']);
      //$this->item['preview'] = '/static/js/ckeditor/filemanager/connectors/php/filemanager.php?mode=preview&path=' . rawurlencode($path);
